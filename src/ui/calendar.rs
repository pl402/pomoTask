...
pub fn render_calendar(app: &App, frame: &mut Frame, area: Rect) {
    ...
    let mut title = match app.config.calendar_range {
        ...
        crate::app::CalendarRange::Week => format!(
            " {} {} ({}) ",
            month_name,
            year,
            app.translate("calendar_range_week")
        ),
        crate::app::CalendarRange::Day => format!(
            " {} {} ({}) ",
            month_name,
            year,
            app.translate("calendar_range_day")
        ),
    };

    // Agrupar estadísticas por fecha completa y hora
    let mut pending_counts = std::collections::HashMap::new();
    let mut done_counts: std::collections::HashMap<NaiveDate, i64> =
    ...
    match app.config.calendar_range {
        crate::app::CalendarRange::Month => {
            ...
            let day_headers = (0..7)
                .map(|i| {
                    ...
                    Cell::from(
                        Line::from(name.chars().take(2).collect::<String>())
            ...
            for week in 0..6 {
                ...
                for day_of_week in 0..7 {
                    if (week == 0 && day_of_week < start_weekday)
                        || current_day_num > days_in_month as u32
                    {
        ...
        crate::app::CalendarRange::Week => {
            let start_of_week = app.calendar_date
                - chrono::Duration::days(app.calendar_date.weekday().num_days_from_sunday() as i64);

            // Header: Días de la semana
            let day_headers = std::iter::once(Cell::from(""))
                .chain((0..7).map(|i| {
                    let date = start_of_week + chrono::Duration::days(i as i64);
                    let name =
                        app.translate(&format!("day_{}", date.weekday().num_days_from_sunday()));
                    Cell::from(
                        Line::from(format!(
                            "{} {:02}",
            ...
            for h in start_h..start_h + 8 {
                let mut cells = vec![Cell::from(format!("{:02}:00", h))
                ...
                for i in 0..7 {
                    ...
                    cells.push(
                        Cell::from(Line::from(symbol).alignment(Alignment::Center)).style(style),
                    );
                }
    ...
    let legend = match app.config.calendar_view {
        crate::app::CalendarView::Standard => Line::from(vec![
            sq(Palette::red(app)),
            Span::styled(" pend ", dim),
            sq(Palette::green(app)),
            Span::styled(" ok ", dim),
            sq(Palette::blue(app)),
            Span::styled(" act ", dim),
        ]),
        crate::app::CalendarView::Heatmap => Line::from(vec![
            Span::styled("− ", dim),
            sq(Palette::overlay0(app)),
            sq(Palette::blue(app)),
            sq(Palette::mauve(app)),
            sq(Palette::green(app)),
            Span::styled(" + ", dim),
        ]),
        crate::app::CalendarView::Progress => Line::from(vec![
            Span::styled("󰄲", Style::default().fg(Palette::green(app))),
            Span::styled(" ok ", dim),
            Span::styled("󰄱", Style::default().fg(Palette::yellow(app))),
            Span::styled(" parc ", dim),
        ]),
    };
...
fn render_day_cell(
    ...
    match app.config.calendar_view {
        crate::app::CalendarView::Standard => {
            ...
            if *pending > 0 {
                spans.push(Span::styled("█", Style::default().fg(Palette::red(app))));
            } else {
                spans.push(Span::raw(" "));
            }
            if *done > 0 {
            ...
            if *created > 0 {
                spans.push(Span::styled("█", Style::default().fg(Palette::blue(app))));
            } else {
                spans.push(Span::raw(" "));
            }
        ...
        crate::app::CalendarView::Progress => {
            ...
            if total > 0.0 {
                ...
                if ratio >= 1.0 {
                    content.push('󰄲');
                    style = style.fg(Palette::green(app));
                } else if ratio > 0.0 {
                    content.push('󰄱');
                    style = style.fg(Palette::yellow(app));
                } else {
                    content.push(' ');
                    style = style.fg(Palette::red(app));
                }
            } else {
                content.push(' ');
            }
