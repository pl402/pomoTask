...
use crate::app::{App, AppMode, DatePreset, InputField};
use crate::ui::palette::Palette;
use crate::ui::{anim_rect, centered_rect};

pub fn render_input_modal(app: &App, frame: &mut Frame) {
    let area = anim_rect(app, centered_rect(70, 70, frame.size()));
    frame.render_widget(Clear, area);
    let title_key = match app.mode {
        AppMode::SubtaskInput => "subtask_title",
        AppMode::Edit => "edit_title",
        _ => "input_title",
    };
    ...
    let caret = |focused: bool, text: &str| {
        if focused {
            format!("{}▏", text)
        } else {
            text.to_string()
    ...
    let ftitle = |focused: bool, key: &str| {
        if focused {
    ...
    frame.render_widget(
        Paragraph::new(title_text).scroll((0, title_scroll)).block(
            Block::default()
                .title(ftitle(is_title, "field_title"))
                .borders(Borders::ALL)
                .border_type(BorderType::Rounded)
    ...
    frame.render_widget(
        Paragraph::new(notes_text).scroll((0, notes_scroll)).block(
            Block::default()
                .title(ftitle(is_notes, "field_notes"))
                .borders(Borders::ALL)
                .border_type(BorderType::Rounded)
    ...
    if app.mode == AppMode::Input {
        ...
        frame.render_widget(
            Paragraph::new(list_text)
                ...
                .block(
                    Block::default()
                        .title(ftitle(is_list, "list_selection"))
                        .borders(Borders::ALL)
                        .border_type(BorderType::Rounded)
    ...
    for (idx, (preset, label)) in presets.iter().enumerate() {
        ...
        let block = Block::default()
            .borders(Borders::ALL)
            .border_type(BorderType::Rounded)
            .border_style(if is_selected && app.focused_input == InputField::Due {
                Style::default().fg(Palette::yellow(app))
            } else {
                Style::default().fg(Palette::surface0(app))
            });
        frame.render_widget(
    ...
    let mut due_title = vec![Span::raw(format!(
        "{}{} ",
        prefix,
        app.translate("due_date")
    ))];
    if app.input_due.trim().is_empty() {
        due_title.push(Span::styled(
            format!("{} ", app.translate("due_date_hint")),
            Style::default().fg(Palette::overlay0(app)),
        ));
    } else if let Some(dt) = App::parse_due_date(&app.input_due) {
        due_title.push(Span::styled(
            format!("✓ {} ", app.format_due_date(dt)),
            Style::default().fg(Palette::green(app)),
        ));
    } else {
        due_title.push(Span::styled(
            format!("✗ {} ", app.translate("date_invalid")),
            Style::default().fg(Palette::red(app)),
        ));
    }
    frame.render_widget(
        Paragraph::new(date_input_text)
            .scroll((0, due_scroll))
            .block(
                Block::default()
                    .title(Line::from(due_title))
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
    ...
    let input_hint = if app.focused_input == InputField::Due {
        if app.config.language == crate::app::Language::Spanish {
            " ←→: Cambiar Fecha | ENTER: Guardar | ESC: Salir ".to_string()
        } else {
            " ←→: Change Date | ENTER: Save | ESC: Exit ".to_string()
        }
    } else if app.focused_input == InputField::List {
        if app.config.language == crate::app::Language::Spanish {
            " ←→: Cambiar Lista | ENTER: Guardar | ESC: Salir ".to_string()
        } else {
            " ←→: Change List | ENTER: Save | ESC: Exit ".to_string()
        }
    } else {
        app.translate("input_hint")
    };
    frame.render_widget(
        Paragraph::new(input_hint)
            .alignment(Alignment::Center)
...
pub fn render_help_modal(app: &App, frame: &mut Frame) {
    ...
    let groups: [(String, Vec<(&str, String)>); 5] = [
        (
            app.translate("help_group_timer"),
            vec![
        ...
        (
            app.translate("help_group_tasks"),
            vec![
        ...
        (
            app.translate("help_group_views"),
            vec![
        ...
        (
            app.translate("help_group_general"),
            vec![
    ...
    for (gname, keys) in groups.iter() {
        rows.push(Row::new(vec![
            Cell::from(""),
            Cell::from(format!("─ {} ─", gname)).style(
                Style::default()
                    .fg(Palette::overlay0(app))
...
pub fn render_settings_modal(app: &App, frame: &mut Frame) {
    ...
    let block = Block::default()
        .title(format!(" {} ", app.translate("settings_title")))
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded)
        .border_style(Style::default().fg(Palette::yellow(app)));
    ...
    let items: [(String, String); 10] = [
        (
            format!("⏱  {}", app.translate("settings_focus")),
            format!("{} min", app.config.focus_duration / 60),
        ),
        (
            format!("☕  {}", app.translate("settings_short")),
            format!("{} min", app.config.short_break_duration / 60),
        ),
        (
            format!("🌙  {}", app.translate("settings_long")),
            format!("{} min", app.config.long_break_duration / 60),
        ),
        (
            format!("🌐  {}", app.translate("settings_lang")),
            match app.config.language {
                crate::app::Language::Spanish => "Español".to_string(),
                crate::app::Language::English => "English".to_string(),
            },
        ),
        (
            format!("🎨  {}", app.translate("settings_theme")),
            app.config.theme.name().to_string(),
        ),
        (
            format!("📅  {}", app.translate("settings_calendar_view")),
        ...
        (
            format!("🔭  {}", app.translate("settings_calendar_range")),
        ...
        (
            format!("🧹  {}", app.translate("settings_retention")),
            match app.config.stats_retention {
        ...
        (
            format!("🔄  {}", app.translate("settings_sync_interval")),
            if app.config.sync_interval_minutes == 0 {
        ...
        (
            format!("🚪  {}", app.translate("settings_logout")),
            String::new(),
        ),
    ];
    ...
    for (gname, range) in groups.iter() {
        ...
        for i in range.clone() {
            ...
            let value_disp = if selected && !value.is_empty() {
                format!("◄ {} ►", value)
            } else {
                value.clone()
            };
            ...
            rows.push(
                Row::new(vec![Cell::from(label.clone()), Cell::from(value_disp)]).style(style),
            );
        }
    }
...
pub fn render_logout_confirm_modal(app: &App, frame: &mut Frame) {
    ...
    frame.render_widget(
        Block::default()
            .title(format!(" {} ", app.translate("logout_confirm_title")))
            .borders(Borders::ALL)
            .border_type(BorderType::Rounded)
            .border_style(Style::default().fg(Palette::red(app))),
        area,
    );
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        ...
        .split(area);
    frame.render_widget(
        Paragraph::new(app.translate("logout_confirm_msg")).alignment(Alignment::Center),
        chunks[1],
    );
    frame.render_widget(
        Paragraph::new(app.translate("confirm_hint"))
            .alignment(Alignment::Center)
            .style(Style::default().fg(Palette::overlay0(app))),
        chunks[2],
    );
...
pub fn render_confirm_modal(app: &App, frame: &mut Frame) {
    ...
    if let Some(t) = task {
        ...
        frame.render_widget(
            Block::default()
                .title(format!(" {} ", app.translate("confirm_title")))
                .borders(Borders::ALL)
                .border_type(BorderType::Rounded)
                .border_style(Style::default().fg(Palette::mauve(app))),
            area,
...
pub fn render_list_selector(app: &App, frame: &mut Frame) {
    ...
    let items: Vec<ListItem> = app
        ...
        .enumerate()
        .map(|(i, l)| {
            let s = if i == app.selected_list_idx {
                Style::default()
                    .fg(Palette::base(app))
                    .bg(Palette::mauve(app))
            } else {
                Style::default().fg(Palette::text(app))
            };
