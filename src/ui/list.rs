...
pub fn render_left_panel(app: &App, frame: &mut Frame, area: Rect) {
    ...
    let gauge_fg = if app.timer_mode == TimerMode::Focus {
    ...
    let label = format!(
        ...
        match app.timer_mode {
            TimerMode::Focus => app.translate("focus"),
            _ => app.translate("break"),
        },
    ...
    frame.render_widget(
        Gauge::default()
            .block(
                Block::default()
                    .title(format!(" {} ", app.translate("timer")))
                    .borders(Borders::ALL)
    ...
    let header_titles: Vec<String> = if is_all_view {
        vec![
            format!(" {} ", app.translate("col_task")),
            format!(" 📋 {} ", app.translate("col_list")),
            format!(" 🕒 {} ", app.translate("col_created")),
            format!(" 📅 {} ", app.translate("col_due")),
            " 🍅 ".to_string(),
        ]
    } else {
        vec![
    ...
    let rows: Vec<Row> = app
        ...
        .map(|(i, task)| {
            ...
            let is_subtask = if let Some(pid) = &task.parent_id {
                app.tasks.iter().any(|t| &t.id == pid)
            } else {
                false
            };
    ...
    if app.loading
        && app.tasks.is_empty()
        && app.marking_done_task_id.is_none()
        && app.creating_task_temp_id.is_none()
    {
